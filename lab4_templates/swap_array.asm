...
doSwap:
        ...
        li $t0 4 #x=1 * 4 = 4 bytes
        li $t1 44 #y=11 * 4 = 44 bytes

        li $t2 24 #c=6 *4 = 24 bytes
        la $t6 myArray

        addu $t0 $t6 $t0 # get the exact spot for x in mem
        addu $t1 $t6 $t1 # get the exact spot for y in mem
        addu $t2 $t6 $t2
loop:
        # x < c. continue
	slt $t7 $t0 $t2
	beq $t7 $zero exit
        ...
        lw $t4 0($t0) # int temp = myArray[x]
        lw $t5 0($t1)# int temp2 = myArray[y] broke into two lines
        sw $t5 0($t0)# myArray[x] = myArray[y]; 
        sw $t4 0($t1)# myArray[y] = temp; 
